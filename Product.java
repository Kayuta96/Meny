...
public class Product {                                                                                                // Conny Fu conny.fu@iths.se
    ...
    // Setters - Sätter värden till name, price, productGroup och priceKG //KÄNNS OM ATT MINA SETTERS OCH GETTERS ANVÄNDS EJ?
    ...
    public double getTotalPrice(int quantity) {
        if (salePrice > 0 && quantity >= saleQuantity) {
            int discountQuantity = quantity / saleQuantity;
            int regularQuantity = quantity % saleQuantity;
            return (discountQuantity * salePrice) + (regularQuantity * price);
        } else {
